package com.thecodingcult.truecommunity.community.presentation.IMPComps.Funds.Components
...
fun FundraisingBottomSheet(
    ...
    val bottomSheetHeight = (screenHeight * 0.7f).coerceAtMost(600.dp)
    var currentQuestionIndex by remember { mutableIntStateOf(-1) }
    val questions = listOf(
        "Enter required goal amount",
        "Upload Documents or Image (Optional)",
    ...
    Column(
        ...
        ExposedDropdownMenuBox(
            ...
            TextField(
                value = selectedCategory ?: "I am Raising funds for",
                onValueChange = {},
                readOnly = true,
                trailingIcon = {
                    Icon(
                    ...
                    onSendMessage = {
                        if (messageText.text.isNotBlank()) {
                            coroutineScope.launch {
                                messages.add(Message(messageText.text, isSentByUser = true))
                                answers[currentQuestionIndex] = messageText.text
    ...
    if (showDialog) {
        AlertDialog(
            ...
            title = { Text("Your request is pending for verification") },
            text = { Text("Once your request is approved, we will inform you.") },
            confirmButton = {
